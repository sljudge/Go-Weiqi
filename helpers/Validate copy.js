class Validate {
    constructor(board, toPlay, previousBoardPosition, ko) {
        ...
        this.currentChain = new Set
        this.chainEyesAndLibs = new Set
        this.inSeki = new Set
    ...
    countEyesAndLibs(i, owner, board = this.board) {
        ...
        return { id: i, eyesAndLibs: this.chainEyesAndLibs, eyes: this.eyes, libs: this.libs, hasBeenChecked: this.hasBeenChecked, currentChain: this.currentChain, owner: owner, board: board }
    }

    handleCountEyesAndLibs(id, owner, board) {
        if (this.chainEyesAndLibs.has(id)) {
            return;
        }
        ...
        if (board[id] === '.') {
            this.areas.forEach(area => {
                if (area.chain.includes(id)) {
                    if (area.owner === owner) {
                        this.chainEyesAndLibs.add(...area.chain)
                        this.eyes.push(area.chain)
                        this.countEyesAndLibs(id, owner, board)
                    } else if (area.owner === '.') {
                        this.libs.push(id)
                        this.chainEyesAndLibs.add(id)
                    }
                }
            })
        } else if (board[id] === owner && !this.currentChain.has(id)) {
            this.currentChain.add(id)
            this.countEyesAndLibs(id, owner, board)
        }
    }
    ...
    deadBehindEnemyLines(id) {
        ...
        if (this.chainEyesAndLibs.size == 2) {
    ...
    checkForSeki(id) {
        ...
        const chainEyesAndLibs = this.chainEyesAndLibs
        ...
        if (eyes.length == 0) {
            ...
            const checkTwoLiberties = (playingAs, eyesAndLibs, board) => {
                ...
                return !this.hasLiberties(eyesAndLibs[0], playingAs, board) && !this.hasLiberties(eyesAndLibs[1], playingAs === 'x' ? 'o' : 'x', board)
            }
        ...
        if (eyes.length == 1 && eyes[0].length == 1) {
            ...
            const checkOneEyeEach = (playingAs) => {
                ...
                console.log('SEKI count', libs[0], this.countEyesAndLibs(libs[0], tempBoard[libs[0]], tempBoard))
                // if one eye left of length one then check for liberties
                if (this.eyes.length == 1 && this.eyes[0].length == 1) {
                    tempBoard.replaceAt(this.eyes[0], playingAs === 'x' ? 'o' : 'x')
            ...
            console.log('SEKI has been checked 2', this.currentChain, this.hasBeenChecked)
            return blackCheck && whiteCheck
        }
    }
    ...
    scoreDraftCaptures(i) {
        ...
        if (deadBehindEnemyLines) {
            if (this.board[i] === 'o') {
                this.blackDraftCaptures += this.currentChain.size
            } else {
                this.whiteDraftCaptures += this.currentChain.size
            }
